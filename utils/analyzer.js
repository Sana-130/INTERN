...
const repoDir = '../downloads/repo_files/';
...
async function downloadZip(owner, repo) {
  ...
  async function extractZip(repo) {
    ...
    const target = path.resolve(path.join(repoDir, `ex-${repo}`));
    //const targetFolder = path.join(downloadDir, `ex-${repo}`);
    const targetFolder = path.join(repoDir, `ex-${repo}`);
    try {
      try {
  ...
  async function getLOC(repo){
    try {
      const command = `scc -c --format json downloads/repo_files/ex-${repo}`;
  ...
  async function cpDetect(repo, lang){
    try{
      ...
      let command = `jscpd -s downloads/repo_files/ex-${repo} -f ${langL}`;
      const { stdout, stderr } = await exec(command);
  ...
  function removeFiles(repo) {
    ...
    return new Promise((resolve, reject) => {
      ...
      fs.unlink(target, (err) => {
        if (err) {
          reject(`Error removing file: ${err.message}`);
        } else {
